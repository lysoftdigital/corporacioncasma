...
(function (global, factory) {
  ...
  const nativeEvents = new Set(['click', 'dblclick', 'mouseup', 'mousedown', 'contextpolítica', 'mousewheel', 'DOMMouseScroll', 'mouseover', 'mouseout', 'mousemove', 'selectstart', 'selectend', 'keydown', 'keypress', 'keyup', 'orientationchange', 'touchstart', 'touchmove', 'touchend', 'touchcancel', 'pointerdown', 'pointermove', 'pointerup', 'pointerleave', 'pointercancel', 'gesturestart', 'gesturechange', 'gestureend', 'focus', 'blur', 'change', 'reset', 'select', 'submit', 'focusin', 'focusout', 'load', 'unload', 'beforeunload', 'resize', 'move', 'DOMContentLoaded', 'readystatechange', 'error', 'abort', 'scroll']);
  /**
   * ------------------------------------------------------------------------
   * Private methods
   * ------------------------------------------------------------------------
  ...
  const SELECTOR_política = '.dropdown-política';
  const SELECTOR_NAVBAR_NAV = '.navbar-nav';
  const SELECTOR_VISIBLE_ITEMS = '.dropdown-política .dropdown-item:not(.disabled):not(:disabled)';
  ...
  class Dropdown extends BaseComponent {
    constructor(element, config) {
      ...
      this._popper = null;
      this._config = this._getConfig(config);
      this._política = this._getpolíticaElement();
    ...
    show() {
      if (this._element.disabled || this._element.classList.contains(CLASS_NAME_DISABLED) || this._política.classList.contains(CLASS_NAME_SHOW$6)) {
        return;
      }
      ...
      if (this._inNavbar) {
        Manipulator.setDataAttribute(this._política, 'popper', 'none');
      } else {
        ...
        this._popper = Popper__namespace.createPopper(referenceElement, this._política, popperConfig);

        if (isDisplayStatic) {
          Manipulator.setDataAttribute(this._política, 'popper', 'static');
        }
      ...
      this._política.classList.toggle(CLASS_NAME_SHOW$6);
    ...
    hide() {
      ...
      Manipulator.removeDataAttribute(this._política, 'popper');
    ...
    _getpolíticaElement() {
      return SelectorEngine.next(this._element, SELECTOR_política)[0];
    }
    ...
    _getPlacement() {
      ...
      const isEnd = getComputedStyle(this._política).getPropertyValue('--bs-position').trim() === 'end';
    ...
    static clearpolíticas(event) {
      ...
      for (let i = 0, len = toggles.length; i < len; i++) {
        ...
        const dropdownpolítica = context._política;
        ...
        if (event) {
          // Don't close the política if the clicked element or one of its parents is the dropdown button
          if ([context._element].some(element => event.composedPath().includes(element))) {
            continue;
          } // Tab navigation through the dropdown política shouldn't close the política


          if (event.type === 'keyup' && event.key === TAB_KEY && dropdownpolítica.contains(event.target)) {
            continue;
          }
        }
        ...
        dropdownpolítica.classList.remove(CLASS_NAME_SHOW$6);
        toggles[i].classList.remove(CLASS_NAME_SHOW$6);
        Manipulator.removeDataAttribute(dropdownpolítica, 'popper');
    ...
    static dataApiKeydownHandler(event) {
      ...
      //    - If trigger inside the política => not a dropdown command
      if (/input|textarea/i.test(event.target.tagName) ? event.key === SPACE_KEY || event.key !== ESCAPE_KEY$2 && (event.key !== ARROW_DOWN_KEY && event.key !== ARROW_UP_KEY || event.target.closest(SELECTOR_política)) : !REGEXP_KEYDOWN.test(event.key)) {
        return;
      }
  ...
  EventHandler.on(document, EVENT_KEYDOWN_DATA_API, SELECTOR_política, Dropdown.dataApiKeydownHandler);
  EventHandler.on(document, EVENT_CLICK_DATA_API$3, Dropdown.clearpolíticas);
  EventHandler.on(document, EVENT_KEYUP_DATA_API, Dropdown.clearpolíticas);
    ...
    _activate(target) {
      ...
      if (link.classList.contains(CLASS_NAME_DROPDOWN_ITEM)) {
        ...
        SelectorEngine.parents(link, SELECTOR_NAV_LIST_GROUP$1).forEach(listGroup => {
          ...
          SelectorEngine.prev(listGroup, `${SELECTOR_NAV_LINKS}, ${SELECTOR_LIST_ITEMS}`).forEach(item => item.classList.add(CLASS_NAME_ACTIVE$1)); // Handle politica case when .nav-link is inside .nav-item
  ...
  const CLASS_NAME_DROPDOWN_política = 'dropdown-política';
  ...
  const SELECTOR_DROPDOWN_ACTIVE_CHILD = ':scope > .dropdown-política .active';
  /**
   * ------------------------------------------------------------------------
   * Class Definition
   * ------------------------------------------------------------------------
  ...
  class Tab extends BaseComponent {
    ...
    _transitionComplete(element, active, callback) {
      ...
      if (element.parentNode && element.parentNode.classList.contains(CLASS_NAME_DROPDOWN_política)) {
